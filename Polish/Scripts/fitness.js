function quantitySubmit(){
    ...
    $('#form').append('<button class="btn btn-primary" id="endButton" type="submit" name="submit">Zakończyć</button>');
}
function createExercise(number){
  ...
  let exText = document.createTextNode("Ćwiczenie:");
  const exerciseInput = document.createElement("input");
  exerciseInput.classList.add('form-control');

  let amText = document.createTextNode("Ilość:");
  const amountInput = document.createElement("input");
  amountInput.classList.add('form-control');

  let timeText = document.createTextNode("Czas:");
